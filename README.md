...
**От списка электротоваров до проверенного комплекта — в одном диалоге.**
...
Опубликованная версия общедоступна: для проверки не нужны регистрация, установка или собственные API-ключи. Это самостоятельная демонстрационная площадка с данными EKT; реальный заказ в магазине она не оформляет.
...
**Комплект AI** принимает описание задачи, список закупки или фотографию, находит товары в каталоге EKT и помогает собрать проверяемое предложение. Пользователь видит источники, ограничения и итоговую сумму, а перед изменением корзины подтверждает состав.
...
| Помощь с покупкой           | «Дорого», «Нужно срочно», «Согласовать закупку», «Подойдёт ли мне?» — проверка вариантов или подготовка текста для согласования |
...
Ссылки на сертификаты отображаются, **если они есть в данных поставщика**. В проверенных 200 карточках API такие поля не обнаружены; отсутствующие документы не подменяются вымышленными ссылками.
...
- **Помощь с целым заказом.** При нехватке местного остатка показываются доступные варианты комплектации.
...
- **Поддержка процесса закупки.** Текст для согласования, спецификация и CSV помогают передать результат руководителю или менеджеру.
...
Это реализованные свойства прототипа. Рост конверсии и экономия рабочего времени в реальном магазине пока не измерялись.
...
- **Выборка, а не весь магазин:** 200 товаров, до четырёх основных позиций за один подбор. Отсутствие товара в выборке не доказывает его отсутствие у EKT.
- **Корзина прототипа:** нет реального заказа, резерва, оплаты, CRM-интеграции или установки виджета на ekt.kz. Для внедрения нужны API корзины и механизм сессии покупателя от партнёра.
- **Хранение:** до 12 строк корзины, срок cookies — 7 дней. Ссылка открывает состав текущего браузера, не переносит его на другое устройство. История чата хранится в памяти вкладки и сбрасывается при перезагрузке или переходе на отдельную страницу корзины. Авторизации и постоянной истории нет.
- **Одновременные изменения:** нет серверной транзакции между несколькими вкладками; последний ответ может заменить cookie корзины. Для реального магазина нужны база данных и атомарные операции.
- **Совместимость:** проверяется ограниченный набор параметров. Обнаружение противоречий относится к номинальному току; решение не заменяет техническую проверку применимости товара.
- **Данные партнёра:** сертификаты в проверенной выборке API не найдены; минимальная общая сумма заказа не подтверждена. Кратность продажи не равна минимальной сумме заказа. Срок и стоимость доставки конкретного заказа не рассчитываются.
- **Фото и AI:** ошибки распознавания возможны; количество заказа по фото товара не назначается автоматически. Скорость зависит от OpenAI и EKT; при недоступности сервисов возможны ошибки.
- **Языки:** казахский поддерживается частично; интерфейс, таблицы, карточки и некоторые готовые ответы остаются на русском.
- **Помощь с покупкой:** тексты для менеджера или руководителя только подготавливаются; пользователь отправляет их сам. Автоматических рассылок, скидок и последующих контактов нет.
- **Защита:** проверки входных данных, подписей и отдельные тесты инъекций реализованы; это не аудит безопасности. Ограничение частоты запросов хранится в памяти серверного экземпляра и не является распределённой защитой.
